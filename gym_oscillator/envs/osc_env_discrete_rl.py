...
class oscillatorEnv_dis(gym.Env):
  ...
  def __init__(self, len_state=250, ep_length=10000, nosc=1000, epsilon=0.03, frrms=0.1,random=False,
    ...
    super(oscillatorEnv_dis, self).__init__()
    ...
    self.pre_x_val = oscillator_cpp.Calc_mfx(self.y)
  ...
  def step(self, action):
      ...
      self.pre_x_val = self.x_val
      #Vectorized form for stable baselines
      
      val = float(action) / 20 - 0.5 #float(action-1) / 10 - 1 
      print('action: ', action)
      ...
      #print('len state: ', self.y_state)
      
      #if sigmoid:
          #arrayed_version = sigmoid(arrayed_version)
          
      return arrayed_version, self.Reward(self.x_val,self.x_state,val), self.done, self.x_val-self.pre_x_val
  ...
  def reset(self):
    ...
    #if sigmoid:
        #arrayed_version = sigmoid(arrayed_version)
    return arrayed_version, self.x_val - self.pre_x_val
