...
#     super(oscillatorEnv_Rec, self).__init__()
...
#       return arrayed_version, self.Reward(self.x_val,self.x_state,self.action_array), self.done, {} 
...
#     penalty = 0
#     for i in range(self.Delta):
#       penalty += 10*np.abs(action_value[i])
...
class oscillatorEnv_Rec(gym.Env):
  ...
  def Reward(self, x,x_state,action_value,baseline = False):
    ...
    return -(x-np.mean(x_state))**2 - 12*np.abs(action_value)
